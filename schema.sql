...
-- ENUM for asset status (enforces valid values at DB level)
...
CREATE TABLE assets (
    ...
    asset_tag       VARCHAR(50)  NOT NULL UNIQUE,          -- e.g. "ASSET-001"
    name            VARCHAR(150) NOT NULL,
    category        asset_category NOT NULL,
    status          asset_status   NOT NULL DEFAULT 'AVAILABLE',
    serial_number   VARCHAR(100),
    current_user    VARCHAR(100),                           -- NULL when unassigned
...
CREATE TABLE assignment_logs (
    ...
    action        VARCHAR(50)  NOT NULL,   -- 'ASSIGNED', 'UNASSIGNED', 'MARKED_REPAIR', 'MARKED_AVAILABLE'
    assigned_to   VARCHAR(100),            -- user receiving the asset (nullable for un-assign)
    performed_by  VARCHAR(100),            -- admin/operator who made the change
    notes         TEXT,
    created_at    TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);
...
-- ============================================================
-- SEED DATA — Sample assets for development/testing
-- ============================================================
...
-- Update seed rows that are already assigned
