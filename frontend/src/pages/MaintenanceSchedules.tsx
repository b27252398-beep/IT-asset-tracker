  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Maintenance Schedules</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage recurring IT tasks and checkups</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100 dark:divide-slate-700">
            <thead className="bg-slate-50/50 dark:bg-slate-900/50">
              <tr>
                ...
                <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Next Due</th>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-start">
                        <div className={`flex-shrink-0 h-10 w-10 rounded-lg flex items-center justify-center ${overdue ? 'bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400' : 'bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400'}`}>
                          <Calendar className="w-5 h-5" />
                        </div>
                        <div className="ml-4">
                          <div className="text-sm font-bold text-slate-900 dark:text-white">{sched.title}</div>
                          {sched.description && <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-xs truncate">{sched.description}</div>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {sched.nextDueDate ? (
                        <div className={`flex items-center text-sm font-medium ${overdue ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-white'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {sched.status === 'ACTIVE' ? (
                        ...
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300"><Clock className="w-3 h-3 mr-1"/> Paused</span>
                      )}
                    </td>
          ...
          {schedules.length === 0 && (
            <div className="py-12 text-center">
              ...
              <h3 className="text-lg font-medium text-slate-900 dark:text-white">No Maintenance Schedules</h3>
              <p className="text-slate-500 dark:text-slate-400 mt-1">Create your first schedule to start tracking recurring tasks.</p>
            </div>
          )}
        </div>
      </motion.div>
              ...
              <div className="bg-white dark:bg-slate-900 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-indigo-100 dark:bg-indigo-500/20 sm:mx-0 sm:h-10 sm:w-10">
                    <Calendar className="h-5 w-5 text-indigo-600 dark:text-indigo-400" aria-hidden="true" />
                  </div>
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 className="text-lg leading-6 font-semibold text-slate-900 dark:text-white" id="modal-title">Create Schedule</h3>
            ...
            <div className="relative z-10 inline-block align-bottom bg-white dark:bg-slate-900 rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-100 dark:border-slate-700">
              <div className="bg-white dark:bg-slate-900 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 className="text-lg leading-6 font-semibold text-slate-900 dark:text-white" id="modal-title">Edit Schedule</h3>
