...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string; icon: React.ReactNode }> = {
    OPEN:        { label: 'Open',        cls: 'bg-amber-100 text-amber-800',   icon: <Clock className="w-3 h-3 mr-1" /> },
    ACCEPTED:    { label: 'Accepted',    cls: 'bg-indigo-100 text-indigo-800', icon: <ThumbsUp className="w-3 h-3 mr-1" /> },
    REJECTED:    { label: 'Rejected',    cls: 'bg-red-100 text-red-700',       icon: <ThumbsDown className="w-3 h-3 mr-1" /> },
    ASSIGNED:    { label: 'Assigned',    cls: 'bg-purple-100 text-purple-800', icon: <Send className="w-3 h-3 mr-1" /> },
    IN_PROGRESS: { label: 'In Progress', cls: 'bg-blue-100 text-blue-800',     icon: <PlayCircle className="w-3 h-3 mr-1" /> },
    RESOLVED:    { label: 'Resolved',    cls: 'bg-emerald-100 text-emerald-800', icon: <CheckCircle className="w-3 h-3 mr-1" /> },
  };
  const s = map[status] || { label: status, cls: 'bg-slate-100 text-slate-700', icon: null };
  return (
...
function PriorityBadge({ priority }: { priority: string }) {
  const map: Record<string, { label: string; cls: string; icon: React.ReactNode }> = {
    CRITICAL: { label: 'Critical', cls: 'bg-red-100 text-red-800',    icon: <AlertOctagon className="w-3 h-3 mr-1" /> },
    HIGH:     { label: 'High',     cls: 'bg-orange-100 text-orange-800', icon: <AlertTriangle className="w-3 h-3 mr-1" /> },
    MEDIUM:   { label: 'Medium',   cls: 'bg-amber-100 text-amber-800',  icon: <AlertCircle className="w-3 h-3 mr-1" /> },
    LOW:      { label: 'Low',      cls: 'bg-blue-100 text-blue-800',    icon: <Info className="w-3 h-3 mr-1" /> },
  };
  const p = map[priority] || { label: priority, cls: 'bg-slate-100 text-slate-700', icon: null };
  return (
  ...
  if (status === 'REJECTED') {
    return (
      <div className="flex items-center gap-1 mt-2">
        <span className="text-xs text-red-600 font-semibold flex items-center gap-1"><ThumbsDown className="w-3 h-3"/>Ticket was rejected</span>
      </div>
    );
  }
    ...
    <div className="flex items-center gap-1 mt-2">
      {steps.map((s, i) => (
        <React.Fragment key={s}>
          <div className={`h-1.5 flex-1 rounded-full transition-colors ${i <= current ? 'bg-indigo-500' : 'bg-slate-200'}`} />
        </React.Fragment>
      ))}
    </div>
  );
    ...
    <motion.div variants={itemVariants} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="p-5 border-b border-slate-100 bg-slate-50/50">
        <div className="flex justify-between items-start gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="text-base font-bold text-slate-900 leading-tight truncate">{req.title}</h3>
            <p className="text-xs text-slate-500 mt-0.5">Asset: <span className="font-medium text-slate-700">{req.asset?.name || 'Unknown'}</span> ({req.asset?.assetTag || '—'})</p>
          </div>
      ...
      <div className="p-5 flex-1 space-y-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-slate-500 text-xs">Reported by</span>
          <span className="font-semibold text-slate-800 text-xs">{req.employee?.name || 'Unknown'}</span>
        </div>
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
          <p className="text-xs text-slate-500 font-semibold uppercase mb-1">Issue</p>
          <p className="text-sm text-slate-700 line-clamp-3">{req.description}</p>
        </div>
        {req.techNote && (
        ...
        {req.resolvedNote && (
          <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100">
            <p className="text-xs text-emerald-700 font-semibold uppercase mb-1">Resolution</p>
      ...
      <div className="p-4 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {userRole === 'ADMIN' && req.status === 'OPEN' && (
            <>
              <button onClick={onAccept} className="flex items-center gap-1 px-2.5 py-1 bg-indigo-100 text-indigo-700 text-xs font-semibold rounded-lg hover:bg-indigo-200 transition-colors">
                <ThumbsUp className="w-3 h-3" />Accept
              </button>
              <button onClick={onReject} className="flex items-center gap-1 px-2.5 py-1 bg-red-100 text-red-700 text-xs font-semibold rounded-lg hover:bg-red-200 transition-colors">
          ...
          {userRole === 'TECH_TEAM' && req.status === 'ASSIGNED' && (
            <button onClick={onProgress} className="flex items-center gap-1 px-2.5 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded-lg hover:bg-blue-200 transition-colors">
              <PlayCircle className="w-3 h-3" />Start Work
            </button>
          )}
          {userRole === 'TECH_TEAM' && req.status === 'IN_PROGRESS' && (
            <button onClick={onResolve} className="flex items-center gap-1 px-2.5 py-1 bg-emerald-100 text-emerald-700 text-xs font-semibold rounded-lg hover:bg-emerald-200 transition-colors">
  ...
  return (
    <motion.div className="space-y-6 pb-12" variants={containerVariants} initial="hidden" animate="visible">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Helpdesk — {roleTitle}</h1>
          <p className="text-sm text-slate-500 mt-1">{roleSubtitle}</p>
        </div>
      ...
      <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs text-slate-500 shadow-sm overflow-x-auto">
        <span className="font-semibold text-slate-700 shrink-0">Workflow:</span>
      ...
      {isAddModalOpen && (
        <Modal title="Report New Issue" onClose={() => setIsAddModalOpen(false)}>
          <form onSubmit={e => { e.preventDefault(); createMutation.mutate(newReq); }} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Reporting Employee</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Affected Asset</label>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={() => setIsAddModalOpen(false)} className="px-4 py-2 border border-slate-300 text-sm font-medium rounded-lg text-slate-700 hover:bg-slate-50">Cancel</button>
      ...
      {rejectTarget && (
        <Modal title={`Reject: "${rejectTarget.title}"`} onClose={() => { setRejectTarget(null); setRejectionReason(''); }}>
          <div className="space-y-4">
            <p className="text-sm text-slate-600">Please provide a reason for rejecting this ticket. The employee will see this reason.</p>
            <div>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => { setRejectTarget(null); setRejectionReason(''); }} className="px-4 py-2 border border-slate-300 text-sm font-medium rounded-lg text-slate-700 hover:bg-slate-50">Cancel</button>
              <button
      ...
      {forwardTarget && (
        <Modal title={`Forward to Tech Team: "${forwardTarget.title}"`} onClose={() => { setForwardTarget(null); setTechNote(''); }}>
          <div className="space-y-4">
            <p className="text-sm text-slate-600">Add a note for the technical team explaining what they need to investigate or fix.</p>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Note for Tech Team</label>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => { setForwardTarget(null); setTechNote(''); }} className="px-4 py-2 border border-slate-300 text-sm font-medium rounded-lg text-slate-700 hover:bg-slate-50">Cancel</button>
              <button
      ...
      {resolveTarget && (
        <Modal title={`Resolve: "${resolveTarget.title}"`} onClose={() => { setResolveTarget(null); setResolvedNote(''); }}>
          <div className="space-y-4">
            <p className="text-sm text-slate-600">Describe what action you took to resolve this issue.</p>
            <div>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => { setResolveTarget(null); setResolvedNote(''); }} className="px-4 py-2 border border-slate-300 text-sm font-medium rounded-lg text-slate-700 hover:bg-slate-50">Cancel</button>
              <button
