...
export default function LoginTech() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      login(token, { ...user, role: 'TECH_TEAM' });
      navigate('/');
    } catch {
    ...
    <div className="min-h-screen flex" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 relative overflow-hidden"
        ...
        <div className="relative z-10">
          ...
          <div className="mt-8 p-4 rounded-xl bg-white dark:bg-slate-900/10 backdrop-blur-sm border border-white/20">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle className="w-4 h-4 text-emerald-200" />
              <span className="text-white text-sm font-semibold">Your Workflow</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-emerald-200">
              <span className="px-2 py-0.5 rounded-full bg-white dark:bg-slate-900/10 border border-white/20">Assigned</span>
              <span>→</span>
              <span className="px-2 py-0.5 rounded-full bg-white dark:bg-slate-900/10 border border-white/20">In Progress</span>
              <span>→</span>
              <span className="px-2 py-0.5 rounded-full bg-white dark:bg-slate-900/10 border border-white/20">Resolved</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-50 dark:bg-slate-900/50">
        <div className="w-full max-w-md">
          ...
          <Link to="/portal" className="inline-flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:text-emerald-400 mb-8 transition-colors">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-8">
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-900 dark:text-white">Tech Team Portal</h1>
                <p className="text-xs text-slate-500 dark:text-slate-400">Technical Support Access</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" required value={username} onChange={e => setUsername(e.target.value)}
                    placeholder="tech"
                    className="w-full pl-10 pr-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-slate-50 dark:bg-slate-900/50" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPass ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)}
                    placeholder="••••••••"
                    className="w-full pl-10 pr-10 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-slate-50 dark:bg-slate-900/50" />
              ...
              <div className="p-3 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/30">
                <p className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold">Demo credentials</p>
                <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">Username: <strong>tech</strong> · Password: <strong>tech123</strong></p>
              </div>
