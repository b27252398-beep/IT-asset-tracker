...
export default function Software() {
  ...
  const getStatusBadge = (status: string, expiryDate: string) => {
    if (status === 'REVOKED') {
      return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-500/20 text-red-800 dark:text-red-300"><XCircle className="w-3 h-3 mr-1"/> Revoked</span>;
    }
    
    if (expiryDate && new Date(expiryDate) < new Date()) {
      return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-500/20 text-red-800 dark:text-red-300"><AlertCircle className="w-3 h-3 mr-1"/> Expired</span>;
    }
    
    return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300"><CheckCircle className="w-3 h-3 mr-1"/> Active</span>;
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Software Licenses</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage software keys, SaaS subscriptions, and seat allocations</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100 dark:divide-slate-700">
            <thead className="bg-slate-50/50 dark:bg-slate-900/50">
              <tr>
                ...
                <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Seats</th>
                ...
                <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Expiry</th>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-slate-900 dark:text-white">{license.name}</div>
                          <div className="text-sm text-slate-500 dark:text-slate-400">{license.publisher}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-900 dark:text-white font-mono bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded inline-block">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="w-full max-w-[150px]">
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-slate-600 dark:text-slate-300 font-medium">{license.seatsAllocated} / {license.seatsTotal} used</span>
          ...
          {licenses.length === 0 && (
            <div className="py-12 text-center">
              ...
              <h3 className="text-lg font-medium text-slate-900 dark:text-white">No software licenses</h3>
              <p className="text-slate-500 dark:text-slate-400 mt-1">Get started by adding your first software license.</p>
            </div>
          )}
        </div>
      </motion.div>
              ...
              <div className="bg-white dark:bg-slate-900 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-indigo-100 dark:bg-indigo-500/20 sm:mx-0 sm:h-10 sm:w-10">
                    <Key className="h-5 w-5 text-indigo-600 dark:text-indigo-400" aria-hidden="true" />
                  </div>
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 className="text-lg leading-6 font-semibold text-slate-900 dark:text-white" id="modal-title">Add Software License</h3>
            ...
            <div className="relative z-10 inline-block align-bottom bg-white dark:bg-slate-900 rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-100 dark:border-slate-700">
              <div className="bg-white dark:bg-slate-900 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 className="text-lg leading-6 font-semibold text-slate-900 dark:text-white" id="modal-title">Edit Software License</h3>
